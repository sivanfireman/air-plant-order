...
//פונקציה לחיצה על כפתור
function btn() { 
    var summery = "הזמנתך הושלמה בהצלחה<br/>" + "שם: " + document.getElementById("fullName").value.toString()+
        "<br>כתובת: " + document.getElementById("adress").value.toString() +
        "<br>בחרת במשלוח: " + checkSelectionShipping().toString() +
        "<br>הצמחים שבחרת: " + selectPlent().toString() +
        "<br>דברים נוספים שבחרת: " + selectAddToShipping().toString() +
        "<br>הערות: " + document.getElementById("message").value.toString();
    //קורא לפונקציית על מנת להדפיס ערכים בתיבה שיצרנו ומכניס את כל הערכים הרלוונטים או מקבל ערכים מפונקציות 

    var alertBox = document.getElementById('customAlert'); //מגדיר תיבת התראה בHTML ואת הנראות בCSS 
    document.getElementById('alertMessage').innerHTML = summery; //משתנה הקולט מה לכתוב בתיבה
